#!/usr/bin/env python3
"""
modlink panel — веб-панель управления.
Работает на Linux (systemd) и Windows 10+ (sing-box напрямую).

Запуск: python panel.py [--port 5000]
Открыть: http://localhost:5000

Зависимости: pip install flask
"""
from __future__ import annotations
import argparse, atexit, json, os, random, shutil, socket, subprocess, sys, time
from pathlib import Path

try:
    from flask import Flask, request, jsonify
except ImportError:
    sys.exit("нужен Flask: pip install flask")
...
app = Flask(__name__)
_sb_proc: subprocess.Popen | None = None   # Windows: handle на процесс sing-box

# ---------------------------------------------------------------------------
def rand_pass(length: int = 10) -> str:
    chars = "abcdefghjkmnpqrstuvwxyz23456789"   # без i/l/o/0/1
...
def load_modems() -> list[dict]:
    ...
    for line in MODEMS_CONF.read_text(encoding="utf-8").splitlines():
        ...
        result.append({"n": n, "password": password})
...
def apply_singbox(modems: list[dict]) -> tuple[bool, str]:
    CONF_DIR.mkdir(parents=True, exist_ok=True)
    cfg_text = json.dumps(gen_singbox_config(modems), indent=2, ensure_ascii=False)
    SB_CONF.write_text(cfg_text, encoding="utf-8")
    if not IS_WIN:
        SB_CONF.chmod(0o600)
    ...
    if IS_WIN:
        ...
        return ok, ("active" if ok else "crashed — проверь конфиг")
    else:
...
@app.route("/api/info")
def api_info():
    return jsonify({"ip": get_server_ip(), "port": PROXY_PORT, "status": get_sb_status()})

@app.route("/api/modems")
def api_get_modems():
    return jsonify(load_modems())
...
def api_apply():
    ...
    if not modems:
        return jsonify({"ok": False, "error": "нет модемов"}), 400
    try:
        parsed = [{"n": int(m["n"]), "password": str(m["password"]).strip()} for m in modems]
    except (KeyError, ValueError) as e:
        return jsonify({"ok": False, "error": str(e)}), 400
    save_modems(parsed)
    ok, status = apply_singbox(parsed)
    if not ok:
        return jsonify({"ok": False, "error": status}), 500
    return jsonify({"ok": True, "status": status})

@app.route("/api/test/<int:n>")
def api_test(n: int):
    modems = load_modems()
    m = next((x for x in modems if x["n"] == n), None)
    if not m:
        return jsonify({"error": f"модем {n} не найден"}), 404
    ...
    return jsonify({"n": n, "exit_ip": exit_ip, "huawei_ok": huawei_ok})
...
<div class="wrap">
  ...
  <div class="bar">
    ...
    <div class="bar-right">
      <button class="btn" onclick="copyClient()" title="Строки для modems.conf клиента">⎘&nbsp;Скопировать</button>
...
function makeRow(n='',pass=''){
  ...
  tr.innerHTML=`
    ...
    <td class="col-login"><input type="text" value="${login}" readonly tabindex="-1"></td>
    <td class="col-pass">
      <div class="pass-wrap">
        <input type="text" value="${p}" placeholder="пароль" autocomplete="off">
        <button class="btn btn-icon" title="Новый пароль" onclick="repass(this)">↺</button>
      </div>
    </td>
...
function onN(inp){
  ...
  const sp=tr.querySelector('.tres');
  if(sp) sp.id=`tr${n}`;
}
...
function addRow(n='',pass=''){
  ...
  const em=tb.querySelector('.empty');
...
function delRow(btn){
  ...
  if(!tb.querySelector('tr'))
    tb.innerHTML='<tr><td colspan="5" class="empty">Нет модемов — нажми + Добавить</td></tr>';
}
...
async function loadInfo(){
  try{
    ...
    srvIp=d.ip; srvPort=d.port;
    document.getElementById('srvIp').textContent=d.ip;
    document.getElementById('srvPort').textContent=d.port;
...
async function doApply(){
  ...
  try{
    const d=await fetch('/api/apply',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({modems:rows})}).then(r=>r.json());
...
async function doTest(btn){
  ...
  try{
    ...
    if(d.exit_ip){
      ...
      sp.title=`exit: ${d.exit_ip}  Huawei .1: ${d.huawei_ok?'OK':'нет'}`;
    } else {
      sp.className='tres fail';
      sp.textContent=d.error||'нет ответа';
    }
...
function copyClient(){
  ...
  const lines=rows.map(m=>`${m.n}  ${srvIp}:${srvPort}:modem-${m.n}:${m.password}`);
...
if __name__ == "__main__":
    ...
    p.add_argument("--host", default="127.0.0.1",
                   help="0.0.0.0 чтобы слушать со всех интерфейсов")
    a = p.parse_args()
    print(f"  modlink panel → http://{a.host}:{a.port}")
